...
LOGGERS = ('csv', 'tb', 'wandb', 'clearml', 'comet', "dvc")  # *.csv, TensorBoard, Weights & Biases, ClearML, DVC
...
class Loggers():
    ...
    def __init__(self, save_dir=None, weights=None, opt=None, hyp=None, logger=None, include=LOGGERS):
        ...
        # DVC
        if dvc and 'dvc' in self.include:
            self.dvc = dvc.Live(save_dvc_exp=True)
        else:
            self.dvc = None
    ...
    def on_pretrain_routine_end(self, labels, names):
        ...
        if self.plots:
            ...
            if self.dvc:
                for x in paths:
                    self.dvc.log_image(x.name, cv2.imread(str(x)))
    ...
    def on_train_end(self, last, best, epoch, results):
        ...
        if self.dvc:
            for f in files:
                self.dvc.log_image(f.stem + ".png", cv2.imread(str(f)))
